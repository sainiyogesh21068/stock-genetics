...
from testingdir.helpers import summarize
...
def run_train(df,dfv,popSize):
    ...
    while generation <= 10:
        ...
        summarize(generation, rul, bestScore ,trn)
        # print(generation)
        generation += 1
    return rul,bestScore
...
if __name__ == "__main__":
    ...
    c1 = 0
    lot_size = 500
    DIR = os.listdir("/home/yogesh/Downloads/JAN2019/JAN")
    DIR.sort()
    # rule = "11011000101001"
    # i = input("DIR : ")
    total_score = 0
    for i in DIR:
        print(i, end="")
        print("\t", end="")
        df1 = pd.read_csv("/home/yogesh/Downloads/JAN2019/JAN/" + str(i) + "/SBIN.txt", sep=",", header=None)
        ...
        dfv_test = dfc[7]
        best_score=0
        rule,best_score = run_train(df_train,dfv_train,pop_size)
        if (best_score < 0):
            print("\t\tNo Profitable Rule Found\t:")
        elif (best_score > 0):
            print("\n")
            print("\t\tRule\t:" ,end=" ")
            print(colored(rule, "blue"), end="")
            # rule = input()
            ...
            print("\t Transactions : " + str(ph.transaction) + "\t")
